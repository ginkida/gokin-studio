...
**Status:** v2.1.3 · GLM/Kimi-only desktop runtime.
...
- **Desktop session-file tree and editor** — Files follows the active chat worktree and uses real tree/treeitem semantics with one roving tab stop. Arrow keys navigate visible entries, Left/Right collapse or expand folders, Home/End jump to the edges, and first-letter search moves without touching the mouse. Clicking ordinary source/text paths in Files, chat markdown, changed-file chips, or Diff opens a bounded UTF-8 editor in that chat's Files pane; `Cmd/Ctrl+S` writes atomically while a content revision detects agent/terminal changes and offers explicit Discard/Override instead of silent clobbering. Each project/session/path owns a distinct editor instance and immutable save payload, so a late save/copy callback cannot replace another selected file or delete its newer cached draft, while overlapping folder expand/retry responses are latest-request-only. File switching pauses visibly while an exact-path save RPC is in flight; external chat/diff artifact navigation passes through the same dirty-file confirmation as a local tree click instead of hiding the editor directly. Right-clicking a file path on those surfaces exposes the same session-resolved actions: attach it to the composer without sending, open it in an installed VS Code/Cursor/Zed, reveal it in Finder/Explorer, or copy its absolute worktree path. Menu resolution, native actions, errors, close, and focus restoration all belong to the exact open request, so finishing an older action cannot close or mutate a newer menu. The header copies the absolute worktree path, Add to chat remains reviewable, unsaved navigation is confirmed, and accidental session switches retain at most eight in-memory drafts for the current app run. `.git`/Studio metadata, non-regular or outward-symlink paths, and traversal are rejected before any external app launches; binary/oversized/read-only files remain non-editable. HTML/PDF/image/video still route to Browser Preview and Office files to Artifacts.
...
- **Local scheduled tasks + background readiness** — interval/daily/weekdays/weekly prompts plus manual-only routines, with pause, edit, delete, and Run now controls. GLM 5.2 and Kimi K3 can also list or create/manage routines through `scheduled_task`; every mutation receives an exact-action approval card showing prompt, cadence, local time, model, and future-run permission mode, even when the project uses Skip. Each run gets its own inspectable child chat and bounded status history while the desktop app is open. The durable run index is the authoritative child owner: a failed publication rolls back the new chat/history/worktree, live rows are never evicted by retention, one routine has at most one live run, and its card is updated only by that exact run ID. Task deletion and provider start are atomically ordered, a missing/corrupt owner stops the child, and terminal status is never reported before its run row commits. Startup repairs interrupted dispatches and stale card summaries from terminal run rows. Task/history reads are latest-request-owned, history polling waits for the previous RPC before scheduling another, and a late Run now response from a replaced workspace cannot navigate the new project. List, history, and action failures retain independent accessible status. An explicit battery-aware setting acquires one ref-counted OS sleep inhibitor for concurrent agent runs and, when any automatic schedule is enabled, keeps the machine awake until schedules are paused/removed or the setting/app is shut down; manual-only routines never hold a wake lease. macOS uses `caffeinate -i`, Windows uses thread-scoped `SetThreadExecutionState`, and Linux uses `systemd-inhibit` when available; live status and acquisition errors are visible in Settings.
...
- **Inline custom visuals and artifact previews** — generated HTML/HTM/SVG, DOCX, XLSX, PPTX, and PDF appear as lazy inline cards in the GLM/Kimi conversation and can expand without leaving the answer. The same files open from Files/changed-file chips in a full preview. Every read follows that chat's isolated worktree. Expanded inline cards and full web previews refresh live, with one sequential read at a time; each result belongs to the exact project/session/path generation, so a late response cannot appear in a replacement chat or file. Full previews support responsive widths and privately deduplicated per-session history (20 versions, 128 MiB/project), and run origin-isolated with external network access requiring opt-in. Content, history, copy, and version actions use the same exact ownership, while Restore invalidates and pauses live reads so older content cannot overwrite the restored preview. Restore changes only the selected worktree; deleting a chat removes its snapshots. Native Office previews are passive and never execute macros, scripts, external links, or embedded objects; downloads preserve the original file.
...
- **Interactive MCP Apps host** — tools can attach standard `ui://` HTML resources to results. Gokin negotiates stable `io.modelcontextprotocol/ui` (2026-01-26), hides app-only tools from GLM/Kimi, validates bounded HTML/result payloads and declared CSP origins, then renders an isolated iframe with the standard lifecycle. Every iframe document is keyed by its HTML plus resource/tool/instance permission identity and must complete initialize within its own load generation. JSON-RPC IDs/methods and tool arguments are bounded; replies, errors, and busy-state cleanup belong to the exact document, `contentWindow`, and request token, so a reload or replacement cannot receive a stale result or have its interaction lock cleared by older work. App-originated `tools/call` requests are restricted to the producing server and `app` visibility, schema/size/rate checked, secret-redacted in the operation preview, and require fresh user approval; links and model-context mutation remain unadvertised.
...
- **File picker modal** (`Ctrl+P` / `Cmd+P`) with a fast name/path filter over the active chat checkout; parallel folder reads are independently latest-request-only, session changes discard every old response, and a failed root scan has an explicit retry
...
- **Shell**: `bash` (foreground or background, with long-output collapse >30 lines), `kill_shell`, `task_output`, `task_stop`
...
- **Agent coordination**: `search_session_transcripts` (read-only literal search across other local chats, optionally scoped to one project or archived chats; available in Plan mode and strips reasoning/tool/document/binary payloads), `session_agent` (list/read/message/rename/archive other local Studio chats, plus user-clickable new-task suggestions; archive always asks first), `delegate` (hand work to another project: `list` the real project IDs and their reachability, `ask` a bounded tool-free question, `run` real work in the target's own checkout, `batch` a bounded fan-out, then `status`/`fetch`/`cancel` the returned run — every start is exact-action reviewed in every mode and never rememberable). Fan-out atomically reserves every target for the exact originating chat before creating a paid turn, so a concurrent run cannot leave a partially-started batch and Stop/Delete can withdraw even a not-yet-activated slot. Registry ownership, durable run publication, and target queue claim are one ordered transition; duplicate run IDs and a second live owner for the same target are rejected, while retention never drops a live row. A delegation `run_id` is scoped to the exact originating project/chat at the model-facing boundary, so one agent cannot inspect or cancel another chat's work; malformed, oversized, duplicate, or incomplete arguments are rejected before anything starts, and paged UTF-8 answers always advance on complete rune boundaries. Stop, Clear, source/target chat deletion, project removal, and disabling computer access commit the exact delegation terminal state before stopping its child, and queue-cleared events include follow-ups removed by that stop. Terminal UI state is irreversible even if a late Started/Progress event overtakes completion. Durable-store read/write failures fail closed: the exact child and its queue are stopped, recoverable bytes are not overwritten, one terminal `storage_error` reaches the caller, stale progress cannot revive it, and batch synthesis refuses incomplete state. `ask_user` surfaces a question to the human in the UI. The retired role-based `ask_agent` Studio bridge is removed rather than retaining its ambiguous “first other project” fallback. Internal specialist responses use single-consumer, exact-owner correlations, so rejected requests cannot leak registry entries and stale handlers cannot answer replacement IDs. Specialist toolsets are immutable for their run: the retired `request_tool` escape hatch cannot add write/shell capabilities to Explore, Plan, Guide, or a dynamic agent, and `tools_list` reports only the exact filtered registry instead of advertising inaccessible tools.
...
- **Reliable WebView downloads** — chat/session/project exports, usage and log CSVs, diagnostics, attachments, artifacts, and the older-bridge backup fallback share one resource-owned Blob downloader. It attaches the download anchor before dispatch, always removes it, and delays object-URL revocation so WebKit cannot lose a download after `click()` returns.
...
- **Backup & Restore** — Settings → Backup & Restore:
  - **Manual backup** writes a portable `.tar.gz` of `~/.config/gokin-studio/` (config + history + drafts + pins + memory + snippets). Desktop builds use a native Save dialog and stream into a private adjacent candidate, `fsync` it, then atomically replace the chosen file—archive bytes never cross the WebView bridge and a failed export preserves an existing backup. A destination inside the config tree, including through a parent symlink, is rejected so the archive cannot consume its own growing candidate. The bounded base64/Blob download remains a compatibility fallback for an older bridge. Root-confined archive walks skip symlinks and special files, so a config-tree link cannot disclose outside data or block on a FIFO. Manual Export and file Restore share a 100 MB boundary; the backend additionally bounds compressed output, the complete expanded tar stream, extracted file bytes, and entry count. Restore verifies the gzip checksum and rejects trailing data or concatenated members before swapping data, so Studio never publishes a backup its own restore path cannot accept.
  - **Auto-backup** — opt-in daily snapshot on app startup, atomically published only after a complete fsynced archive, never replaces an occupied daily or same-second name, retains the last 7, and coalesces concurrent triggers
  - **Consistent bulk operations** — archive reads share a data-tree gate; Cleanup, Import, and Restore take its exclusive side so files cannot be removed or the config directory swapped during an archive walk
  - **Browse + restore** — list and selectively restore from past rollback snapshots (pre-import + pre-restore) AND auto-backups. Desktop file Restore uses a native picker and copies the bounded selection to a private candidate before showing confirmation; Unix unlinks that candidate immediately while retaining its descriptor, while Windows removes crash-left matching temp files after 24 hours. The UI receives only a single-use opaque token, so changing the selected path after review cannot change the confirmed bytes, invalid archives never move live data, and cancel/new selection/shutdown closes the candidate. The bounded FileReader/base64 route remains an older-bridge fallback. Restore is a real keyboard-operable button, progress/success/failure is announced to assistive technology, and one shared UI lock prevents a second config-tree operation from queuing behind an in-flight backup, restore, cleanup, or snapshot deletion. Restore parses staged `config.yaml` with the same syntax and structural bounds as startup before moving current data; collision-resistant staging and safety paths isolate rapid or multi-process restores. A selected rollback directory is atomically claimed before validation, stays cleanup-protected and discoverable after a crash, is hardened to `0700`, and cannot be confused with a replacement planted at its old name. Promotion failures automatically return both current data and the selected snapshot, while any failed rollback reports the exact recovery path instead of hiding it.
- **Cleanup** — manual + auto: removes stale replay logs (>7d manual / >30d auto), old pre-import/pre-restore snapshots, orphaned import-staging dirs, excess auto-backups beyond retention, and terminal delegation chats (>30d manual / >90d auto) only when they are idle, clean, unpinned, draft-free, unused since completion, and belong to an active project. Protected chats keep their durable delegation record. Opt-out toggle.
...
internal/studio/         Wails-bound application layer
  ...
  delegation.go          Cross-project delegation: child session in the target, monitor, cancel, envelope
  delegation_runs.go     Durable delegation record (delegation_runs.json, 0600, atomic, bounded)
  delegation_guard.go    Hop-depth + cycle guard evaluated before every permission gate
  delegation_batch.go    Bounded fan-out with a synthesis turn queued in the caller's own chat
  groups.go              Project groups (member-facing shared context) + DelegationPolicy
  ...
  cleanup.go             Manual + auto cleanup of stale replays/snapshots/staging/backups and safely disposable terminal delegations
...
## WSL projects (Windows)

A project whose directory is a WSL path — `\\wsl.localhost\<Distro>\...` or the older
`\\wsl$\<Distro>\...` — has its files inside a Linux distribution, not on the Windows
filesystem. Gokin Studio detects that from the path alone and runs the project's commands
**inside the distribution** rather than on the host, because that is where the repository,
its toolchain and its dependencies live.

Add one from the sidebar: the add-project form lists the registered distributions, so there
is no UNC path to type by hand.

What is routed into the distribution: the shell tool, git, the integrated terminal, background
tasks, test and verification runs, `gh`, `gopls`, per-chat plugin hooks. Paths in tool output
are shown in their Linux spelling (`/home/you/project`), which is also what you can type back.

Two consequences worth knowing:

- **Tools must be installed in the distribution, not on Windows.** `gh` or `gopls` on the
  Windows PATH is not reachable from inside a distro, so the app checks — and reports —
  availability on the side that will actually run the command.
- **Per-chat git worktrees are skipped** for WSL projects; those sessions share the checkout
  and say so with a "shared checkout" chip.

Preview servers and MCP connectors still run on the host. MCP connectors are configured
app-wide rather than per project, so they have no distribution to belong to.

Before relying on a WSL project, run the probe from a Windows shell — it measures the platform
behaviour this support depends on and prints what would break if any of it differs:

```
go run ./scripts/wslprobe -distro Ubuntu -dir \\wsl.localhost\Ubuntu\home\you\project
```

See `scripts/wslprobe/README.md` for what each check means.
