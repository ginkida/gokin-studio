...
func TestResolveThinkingConfig(t *testing.T) {
	...
	kimiDefault := client.DefaultThinkingBudget("kimi")
	...
	cases := []struct {
		...
		// disabled → sentinel for providers that can turn thinking off. K3 is
		// always-thinking, so a legacy Off value truthfully resolves to low.
		{"glm disabled", "disabled", "glm", "glm-5.1", 0, false, client.ThinkingDisabledSentinel},
		{"kimi disabled", "disabled", "kimi", "kimi-for-coding", 0, false, client.ThinkingDisabledSentinel},
		{"kimi K3 disabled → low", "disabled", "kimi", "k3", 0, true, 4096},
		{"kimi K3 256k disabled → low", "disabled", "kimi", "k3-256k", 0, true, 4096},
		...
		{"glm enabled no budget → 8192", "enabled", "glm", "glm-5.1", 0, true, glmDefault},
		{"kimi enabled no budget → 8192", "enabled", "kimi", "kimi-for-coding", 0, true, kimiDefault},
		...
		{"glm enabled budget 2048", "enabled", "glm", "glm-5.1", 2048, true, 2048},
		{"glm enabled budget 16384", "enabled", "glm", "glm-5.1", 16384, true, 16384},

		// auto: supported providers enable at their tuned default
		{"glm-5.2 auto → max effort budget", "", "glm", "glm-5.2", 0, true, 32768},
		{"glm-5.1 auto → 8192", "", "glm", "glm-5.1", 0, true, glmDefault},
		{"glm-4.7 auto → 8192", "", "glm", "glm-4.7", 0, true, glmDefault},
		{"kimi auto → 8192", "", "kimi", "kimi-for-coding", 0, true, kimiDefault},
		{"kimi K3 auto → high effort budget", "", "kimi", "k3", 0, true, kimiDefault},

		// auto: unsupported models / providers stay off
		{"glm-4.5 auto stays off", "", "glm", "glm-4.5", 0, false, 0},
		{"deepseek auto stays off", "", "deepseek", "deepseek-v4-pro", 0, false, 0},
...
func TestResolveThinkingConfig_GLMAutoVsEnabledConsistent(t *testing.T) {
	_, autoBudget := resolveThinkingConfig("", "glm", "glm-5.1", 0)
	enEnable, enabledBudget := resolveThinkingConfig("enabled", "glm", "glm-5.1", 0)
	if !enEnable {
...
// TypeScript. Supported GLM and Kimi models auto-enable in auto mode.
func TestProjectInfo_ThinkingActiveResolved(t *testing.T) {
	cases := []struct {
		...
		{"glm-5.1 auto → active 8192", "glm", "glm-5.1", "", 0, true, 8192},
		{"glm-5.2 auto → active max effort", "glm", "glm-5.2", "", 0, true, 32768},
		{"glm disabled → off, budget 0", "glm", "glm-5.1", "disabled", 0, false, 0},
		{"glm enabled budget 2048", "glm", "glm-5.1", "enabled", 2048, true, 2048},
		{"glm enabled no budget → 8192", "glm", "glm-5.1", "enabled", 0, true, 8192},
		{"deepseek-v4-pro auto → off", "deepseek", "deepseek-v4-pro", "", 0, false, 0},
		{"kimi auto → active 8192", "kimi", "kimi-for-coding", "", 0, true, 8192},
		{"kimi K3 auto → high effort budget", "kimi", "k3", "", 0, true, 8192},
		{"kimi K3 legacy disabled → active low effort", "kimi", "k3", "disabled", 0, true, 4096},
