...
func TestHealthCheck_OllamaURLMissing(t *testing.T) {
	_ = withTempHistoryDir(t)
	s := NewStudio()
	s.config = defaultConfig()
	s.config.Settings.OllamaURL = ""
	// Make project use ollama.
	p, err := s.AddProject("P", t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	...
	foundWarn := false
	for _, c := range d.Checks {
		if c.Category == "providers" && strings.Contains(c.Name, "Ollama") && c.Status == "warn" {
			foundWarn = true
			break
		}
	}
	if !foundWarn {
		t.Errorf("expected warn check for Ollama URL missing, got: %+v", d.Checks)
	}
}

func TestHealthCheck_OllamaURLSet(t *testing.T) {
	_ = withTempHistoryDir(t)
	s := NewStudio()
	s.config = defaultConfig()
	s.config.Settings.OllamaURL = "http://localhost:11434"
	p, err := s.AddProject("P", t.TempDir())
	if err != nil {
		t.Fatal(err)
	}
	...
	if !foundOK {
		t.Errorf("expected OK check for Ollama URL set, got: %+v", d.Checks)
	}
}
