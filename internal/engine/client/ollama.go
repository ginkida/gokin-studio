...
func (c *OllamaClient) convertHistoryForFallback(history []*genai.Content, results []*genai.FunctionResponse) []api.Message {
	...
	for _, content := range history {
		...
		for _, part := range content.Parts {
			...
			if part.FunctionResponse != nil {
				...
				if val, ok := part.FunctionResponse.Response["result"].(string); ok && val != "" {
					contentStr = val
				} else if val, ok := part.FunctionResponse.Response["content"].(string); ok && val != "" {
					contentStr = val
				} else {
	...
	for _, result := range results {
		...
		if result.Response != nil {
			if val, ok := result.Response["result"].(string); ok && val != "" {
				contentStr = val
			} else if val, ok := result.Response["content"].(string); ok && val != "" {
				contentStr = val
			} else if data, ok := result.Response["data"]; ok && data != nil {
